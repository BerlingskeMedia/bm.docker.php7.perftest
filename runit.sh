...
if [[ -n $NR_APP_NAME && -n $NR_INSTALL_KEY ]] ; then
    ...
    sed -i "s/^newrelic.appname = \"PHP Application\"/newrelic.appname = \"${NR_APP_NAME}\"/" /etc/php/7.0/mods-available/newrelic.ini
    ...
    if [ -n $NR_TRANSACTION_TRACER_ENABLED ]; then
        sed -i "s/^;?newrelic.transaction_tracer.enabled = .*/newrelic.transaction_tracer.enabled = \"${NR_TRANSACTION_TRACER_ENABLED}\"/" /etc/php/7.0/mods-available/newrelic.ini
    fi
    if [ -n $NR_TRANSACTION_TRACER_DETAIL ]; then
        sed -i "s/;?newrelic.transaction_tracer.detail = .*/newrelic.transaction_tracer.detail = \"${NR_TRANSACTION_TRACER_DETAIL}\"/" /etc/php/7.0/mods-available/newrelic.ini
    fi
fi
